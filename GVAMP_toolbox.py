...
def gauss_bernoulli(rho,n,sigma):
...
### functions to update m1x,chi1x and q1x as presented in the equation block (89) ###

def m1x_func(rho,Q1x,mh1x,chih1x,l1,l2,sigma):
    t0 = (sigma*mh1x/Q1x)**2
    t1 = chih1x/Q1x**2
    s = 1/(1+l2/Q1x)
    return(rho*s*mh1x/Q1x*special.erfc(l1/(Q1x*np.sqrt(2*(t1+t0)))))

def chi1x_func(rho,Q1x,mh1x,chih1x,l1,l2,sigma):
    t0 = (sigma*mh1x/Q1x)**2
    t1 = chih1x/Q1x**2
    s = 1/(1+l2/Q1x)
    return(1/Q1x*s*((1-rho)*special.erfc(l1/(Q1x*np.sqrt(2*t1)))+rho*special.erfc(l1/(Q1x*np.sqrt(2*(t1+t0))))))

def q1x_func(rho,Q1x,mh1x,chih1x,l1,l2,sigma):
    t0 = (sigma*mh1x/Q1x)**2
    t1 = chih1x/Q1x**2
    s = 1/(1+l2/Q1x)
...
##### specifc functions for each loss to update m1z, chi1z, q1z from equation block (89) #####
...
def SE_it(lo,l1,l2,rho,sigma,delta0,Q1x,Q1z,mh1x,mh1z,chih1x,chih1z,Tx,Tz,f,asp,damp,d):
        ...
        m1x = m1x_func(rho,Q1x,mh1x,chih1x,l1,l2,sigma)
        chi1x = chi1x_func(rho,Q1x,mh1x,chih1x,l1,l2,sigma)
        q1x = q1x_func(rho,Q1x,mh1x,chih1x,l1,l2,sigma)
        ...
        Q2x = (1/chi1x-Q1x)
        Q2z = (1/chi1z-Q1z)
        mh2x = (m1x/(Tx*chi1x)-mh1x)
        mh2z = (m1z/(Tz*chi1z)-mh1z)
        chih2x = (q1x/(chi1x**2)-(m1x/chi1x)**2/Tx-chih1x)
        chih2z = (q1z/(chi1z**2)-(m1z/chi1z)**2/Tz-chih1z)
        ...
        return(Q1x_new,Q1z_new,mh1x_new,mh1z_new,chih1x_new,chih1z_new,E,ang)
    
##### iterating the SE equations until target precision is reached #####

def SE(lo,l1,l2,rho,sigma,delta0,f,asp,damp,niter,tol,d):
    
    Q1x,Q1z,mh1x,mh1z,chih1x,chih1z = np.random.rand(6)
    Tx = rho*sigma**2
    ...
    while c<niter and conv>tol:
        ...
        Q1x_new,Q1z_new,mh1x_new,mh1z_new,chih1x_new,chih1z_new,E_new,ang_new = SE_it(lo,l1,l2,rho,sigma,delta0,Q1x,Q1z,mh1x,mh1z,chih1x,chih1z,Tx,Tz,f,asp,damp,d)
        conv = max(np.abs((E-E_new)/E),np.abs((ang-ang_new)/ang))
        Q1x,Q1z,mh1x,mh1z,chih1x,chih1z,E,ang = Q1x_new,Q1z_new,mh1x_new,mh1z_new,chih1x_new,chih1z_new,E_new,ang_new
...
# global function to generate a replica prediction

def replica(lo,delta0,rho,sigma,reg,pen,dist,asp_vec,damp,niter,tol):
    n = len(asp_vec)
    E_vec = np.zeros(n)
    ang_vec = np.zeros(n)
    l1= pen*reg
    ...
    for k in range(n):
        ...
        asp = asp_vec[k]
        f = lambda p: MP(p,asp)
        E,ang,conv = SE(lo,l1,l2,rho,sigma,delta0,f,asp,damp,niter,tol,dist)
        E_vec[k] = E
        ang_vec[k] = ang
    return(E_vec,ang_vec)


# simulation for logistic regression

def simulation_logistic(delta0,rho,sigma,reg,asp_vec_n,n_avg,d,pen,dist):
    ...
    for i in range(n_exp):
        ...
        while count<n_avg:
            ...
            if pen==0:
                clf = linear_model.LogisticRegression(penalty='l2',tol=1e-9,C=1/reg,fit_intercept=False,max_iter=100000,solver = 'lbfgs')
            if pen==1:
                clf = linear_model.LogisticRegression(penalty='l1',tol=1e-9,C=1/reg,fit_intercept=False,max_iter=100000,solver = 'liblinear')
            clf.fit(F_train,y_train)
            x_train = clf.coef_
            val = 1/d*x_train@x0/np.sqrt(rho*1/d*np.linalg.norm(x_train)**2)
